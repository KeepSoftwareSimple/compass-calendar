...
Jobs that only run Bun do not install Node. `unit (web)` runs the full web
suite in one leg via `bun run test:web` (`--parallel=2 --no-isolate`). Update
branch protection contexts from `unit-leg (web, 1)` / `(web, 2)` to
`unit-leg (web)` when merging this change.
