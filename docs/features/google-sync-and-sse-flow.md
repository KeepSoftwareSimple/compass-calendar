...
4. Revocation on the mutation path is HTTP 410 `CONNECTION_REVOKED`
   (`event.controller.ts` maps Sync `authorizationRevoked`). SSE attention
   uses the same code with `connectionId` via
   `revokedConnectionServerMessages` in
...
- handles the `CONNECTION_REVOKED` `syncStatusChanged` code consistently with REST 410 payloads
...
2. The SSE attention payload carries `CONNECTION_REVOKED` with
   `connectionId` (see `revokedConnectionServerMessages`).
